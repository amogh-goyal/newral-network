...
"""
    ...
    try:
        ...
        if len(topics) != num_topics:
            ...
            topics = topics[:num_topics] if len(topics) > num_topics else topics + [f"Generic Topic {i + 1}" for i in
                                                                                    range(num_topics - len(topics))]
        return topics, is_programming_related
    except Exception as e:
        print(f"Error parsing LLM response: {e}, response_text: {response_text}")
